...
export const gradeSubmissionService = (courseId, submissionId, data) => {
  // 调用时必须 gradeSubmissionService(courseId, submissionId, {score, comment})
  if (typeof submissionId === 'object' && submissionId !== null && !Array.isArray(submissionId)) {
    // 兼容错误调用，参数错位，自动修正
    data = submissionId
 ...
 * @param {object} data 评分内容，如 { score, comment }
 * @returns {Promise}
 */
